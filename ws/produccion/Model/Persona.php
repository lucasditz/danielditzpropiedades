    ...
    private function addNewPerson($nombre,$apellido,$dni,$fNacimiento,$telefono,$celular,$calle,$nro,$piso,$dpto,$ciudad,$provincia,$id_usuario){
        try{
            ...
            if ($id_direccion!== false && $id_telefono!== false && $id_celular!== false) {
                $query = $this->db->prepare("INSERT INTO persona(nombre,apellido,dni,fecha_nac,id_direccion,id_telefono,id_celular,created,modified,id_user_creation,id_user_modified)
                                        VALUES (:nombre,:apellido,:dni,STR_TO_DATE(:fNacimiento,'%d/%m/%Y'),:id_direccion,:id_telefono,:id_celular,NOW(),NOW(),:id_usuario,:id_usuario)");
