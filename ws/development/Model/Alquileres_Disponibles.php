...
class Alquileres_Disponibles
{
    ...
    public function removeAlquilerDisponible($idInmuebleAlquiler){
        return $this->removeInmuebleAlquiler($idInmuebleAlquiler);
    }

    public function generateAlquilerContract($id_inmuebleAlquiler,$deposito,$honorarios,$alquilerPeriodos,$alquilerServicios,$id_inquilino,$id_garante,$id_usuario){
       return $this->generateInmuebleContract($id_inmuebleAlquiler,$deposito,$honorarios,$alquilerPeriodos,$alquilerServicios,$id_inquilino,$id_garante,$id_usuario);
    }
           ...
           if ($id_direccion !== false) {
               ...
               if ($id_inmueble !== false) {
                   $query = $this->db->prepare("INSERT INTO inm_alquiler(id_inmueble,id_propietario,valor,disponibilidad,removed,created,modified,id_user_creation,id_user_modified)
                                            VALUES (:id_inmueble,:id_propietario,:valor,1,0,NOW(),NOW(),:id_usuario,:id_usuario)");
    ...
    private function generateInmuebleContract($id_inmuebleAlquiler,$deposito,$honorarios,$alquilerPeriodos,$alquilerServicios,$id_inquilino,$id_garante,$id_usuario){
        return $this->addInmuebleContratoPeriodos($id_inmuebleAlquiler,$deposito,$honorarios,$alquilerPeriodos,$alquilerServicios,$id_inquilino,$id_garante,$id_usuario);

    }

    private function addInmuebleContratoPeriodos($id_inmuebleAlquiler,$deposito,$honorarios,$alquilerPeriodos,$alquilerServicios,$id_inquilino,$id_garante,$id_usuario){
        try{
    ...
    /** Remove funtions **/
    private function removeInmuebleAlquiler($idInmuebleAlquiler){
        try{
            $query = $this->db->prepare("UPDATE inm_alquiler
                                         SET removed=1
                                         WHERE id = :idInmuebleAlquiler");

            $query->bindParam(':idInmuebleAlquiler', $idInmuebleAlquiler);
            $query->execute();
